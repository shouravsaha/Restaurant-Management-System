            ...
            <div>
                <table class="food_menu_table">
                    <tr>
                        ...
                        <th class="food_menu_table_th" colspan="2">Action</th>
                    </tr>
                    @foreach ($food_menu_items as $data)
                    <tr class="food_menu_table_tr">
                        <td>{{ $data->title }}</td>
                        <td>{{ $data->price }}</td>
                        <td style="width: 25rem">{{ $data->description }}</td>
                        <td><img class="food_menu_img" src="foodimage/{{ $data->image }}"></td>
                        <td><a class="btn btn-success ml-2 mr-2" href="{{ url('/edit_food_menu', $data->id) }}">Edit</a></td>
                        <td><a class="btn btn-danger mr-2" href="{{ url('/delete_food_menu', $data->id) }}">Delete</a></td>
                    </tr>
                    @endforeach
                </table>
            </div>
